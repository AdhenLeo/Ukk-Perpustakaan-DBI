        ...
        <div class="movies_section layout_padding">
            <div class="container">
                <div class="movies_menu">
                    <ul>
                        <li class="active" style="font-size: 24px"><a href="#">Favorite Books</a></li>
                    </ul>
                </div>
                                ...
                                @forelse ($data->sortByDesc('created_at') as $item)
                                    @if ($item->buku->status_tunggu !== 'tunggu')
                                        <article class="featured__card swiper-slide">
                                            ...
                                            <h2 class="featured__title">{{ $item->buku->judul }}</h2>
                                            <h2 class="featured__title">{{ $item->buku->penulis }}</h2>
                                            <div class="featured__prices">
                                                <span class="featured__discount">{{ $item->buku->penerbit }}</span>
                                                <span class="featured__discount">{{ $item->buku->tahun_terbit }}</span>
                                                <span class="featured__discount">{{ $item->peminjaman->status_peminjaman }}</span>
                                            </div>
                                            ...
                                            <div class="featured__actions">
                                                @if ($favorites->contains('buku_id', $item->buku_id))
                                                    <button class="after-favorited" style="color:#df9911;" onclick="toggleCancelFavorite(this)" data-id="{{ $item->buku_id }}">
                                                        <i class="ri-heart-fill"></i>
                                                    </button>
                                                @else
                                                    <button class="before-favorited" style="color: #df9911;" onclick="toggleFavorite(this)" data-id="{{ $item->id }}">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-4">
                                <img src="{{ asset('storage/buku/' . $item->buku->sampul) }}" class="img-thumbnail w-100 mb-3">
                        ...
                        <button class="btn btn-outline-dark flex-shrink-0 btn-lg" type="button" data-bs-dismiss="modal">Tutup</button>
                        @else
